    ...
    <main class="max-w-7xl mx-auto px-4 py-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 bg-white dark:bg-gray-900 rounded-2xl border border-gray-200/80 dark:border-gray-800/80 shadow-sm">
                <form action="{{ route('charging-points.create.store.step2') }}" method="POST" id="step2Form">
                    @csrf
                    <div class="p-6">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePreview() {
        ...
        if (powerInput && previewPower) {
            previewPower.textContent = powerInput.value ? powerInput.value + ' kW' : 'Non spécifié';
        }
        
        // Update connector preview
        if (connectorTypeSelect && previewConnector) {
            if (connectorTypeSelect.selectedIndex >= 0 && connectorTypeSelect.value) {
        ...
        if (protocolSelect && previewProtocol) {
            if (protocolSelect.selectedIndex >= 0 && protocolSelect.value) {
