                        ...
                        <div class="flex items-center space-x-2">
                            <div class="flex items-center space-x-2">
                                ...
                                <input type="text" id="steve-chargebox-id" placeholder="Ex: BORNE777"
                                       class="px-3 py-1 border border-gray-300 rounded text-sm" value="{{ $steveChargeBoxId ?? $chargingPoint->charge_box_id ?? $chargingPoint->steve_charging_point_id ?? $chargingPoint->serial_number }}">
                            </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div class="space-y-4">
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="space-y-2">
                                    <div class="flex items-center space-x-2">
                                        <label class="text-xs text-gray-500">ID Transaction:</label>
                                        <input type="number" id="transaction-id" placeholder="Auto-détecté" class="px-2 py-1 border border-gray-300 rounded text-xs w-32" min="1">
                                        <button onclick="fetchActiveTransaction()" class="px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs hover:bg-blue-200" title="Récupérer l'ID de transaction actif">
...
<script>
    ...
    // Fonction pour récupérer la transaction active
     function fetchActiveTransaction() {
         const chargeBoxId = document.getElementById('steve-chargebox-id').value;
         const transactionInput = document.getElementById('transaction-id');
         
         fetch(`{{ route('charging-points.enhanced.get-active-transaction', $chargingPoint->id) }}`, {
             method: 'POST',
             headers: {
                 'Content-Type': 'application/json',
                 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
.then(response => {
          ...
          .then(data => {
              if (data.success && data.transaction_id) {
                  ...
                  alert('Aucune transaction active trouvée');
              }
          })
          .catch(error => {
              console.error('Erreur:', error);
...
// Appel API pour exécuter l'action SteVe
         ...
         // Inclure transactionId pour l'action stop
         if (action === 'stop') {
             const transactionId = document.getElementById('transaction-id').value;
             if (transactionId) {
                 bodyData.transactionId = parseInt(transactionId);
             }
         }
            ...
            if (data.success) {
                ...
                switch(action) {
                    ...
                    case 'stop':
                        ...
                        resultText = 'Session de charge arretee avec succes.';
                        if (data.transaction_id) {
                            resultText += '\nTransaction: ' + data.transaction_id;
                        }
                        if (data.ocpp_status) {
                            resultText += '\nStatut SteVe: ' + data.ocpp_status;
                        }
                        break;
                    case 'status':
