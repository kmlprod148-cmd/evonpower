...
<div 
    ...
    @if($dismissible) id="alert-{{ $type }}-{{ uniqid() }}" @endif
>
    <!-- Left accent bar -->
    ...
    <div class="flex items-start gap-3 p-4">
        ...
        @if($dismissible)
            <div class="ml-auto pl-3">
                <button 
                    type="button"
                    onclick="this.closest('[id^=\'alert-{{ $type }}-\']').remove()" 
                    class="inline-flex rounded-md {{ $colors['bg'] }} {{ $colors['text'] }} {{ $colors['hover'] }} p-1.5 focus:outline-none focus:ring-2 focus:ring-offset-0"
                >
