...
@section('page-title', 'Créer un Groupe de Bornes')
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="bg-gradient-to-r from-emerald-600 to-teal-600 rounded-xl p-6 text-white shadow-lg">
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.groups.index') }}" class="text-emerald-200 hover:text-white transition-colors">
                <i class="fas fa-arrow-left"></i>
            </a>
            <div>
                <h1 class="text-xl font-bold">Créer un Groupe de Bornes</h1>
                <p class="text-emerald-100 text-sm mt-0.5">Regroupez des bornes par zone, partenaire ou configuration tarifaire</p>
            </div>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.groups.store') }}" class="space-y-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required placeholder="Ex: Zone Commerce Casablanca"
                           class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-emerald-500 @error('name') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <label class="flex items-center gap-2 p-3 border border-gray-200 dark:border-gray-600 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors {{ old('type', 'business') === 'business' ? 'border-emerald-400 bg-emerald-50 dark:bg-emerald-900/20' : '' }}">
                            <input type="radio" name="type" value="business" {{ old('type', 'business') === 'business' ? 'checked' : '' }} class="text-emerald-600">
                            <div>
                                <p class="text-sm font-medium text-gray-900 dark:text-white">Business</p>
                                <p class="text-xs text-gray-400">Commercial / public</p>
                            </div>
                        </label>
                        <label class="flex items-center gap-2 p-3 border border-gray-200 dark:border-gray-600 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors {{ old('type') === 'private' ? 'border-emerald-400 bg-emerald-50 dark:bg-emerald-900/20' : '' }}">
                            <input type="radio" name="type" value="private" {{ old('type') === 'private' ? 'checked' : '' }} class="text-emerald-600">
                            <div>
                                <p class="text-sm font-medium text-gray-900 dark:text-white">Privé</p>
                                <p class="text-xs text-gray-400">Accès restreint</p>
                            </div>
                        </label>
                    </div>
                ...
                <div>
                    ...
                    <select name="consumption_mode" class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-emerald-500">
                        <option value="prepaid" @selected(old('consumption_mode', 'prepaid') === 'prepaid')>Prépayé</option>
                        <option value="postpaid" @selected(old('consumption_mode') === 'postpaid')>Postpayé</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Ville</label>
                    <input type="text" name="city" value="{{ old('city') }}" placeholder="Casablanca, Rabat..."
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="2" placeholder="Description du groupe..."
                              class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-emerald-500">{{ old('description') }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            <h2 class="text-base font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                ...
                Étape 2 — Propriétaire du groupe
            </h2>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    @if($userPartner)
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Le groupe sera associé à votre profil partenaire</p>
                    @else
                ...
                <div>
                    ...
                    <select name="integrator_id" class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-emerald-500">
                        ...
                        @foreach($integrators as $integrator)
                            <option value="{{ $integrator->id }}" @selected(old('integrator_id') == $integrator->id)>{{ $integrator->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mb-3">Sélectionnez un ou plusieurs plans tarifaires pour ce groupe</p>
                ...
                @foreach($pricingPlans as $plan)
                    <label class="flex items-center gap-3 p-3 border border-gray-200 dark:border-gray-600 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        <input type="checkbox" name="pricing_plan_ids[]" value="{{ $plan->id }}"
                               {{ in_array($plan->id, old('pricing_plan_ids', [])) ? 'checked' : '' }}
        ...
        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('admin.groups.index') }}" class="px-5 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 transition-colors">
                Annuler
            </a>
            <button type="submit" class="px-5 py-2.5 text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg transition-colors shadow-sm">
                <i class="fas fa-save mr-1.5"></i> Créer le groupe
            </button>
        </div>
    </form>
</div>
