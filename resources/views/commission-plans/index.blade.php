...
@section('title', 'Plans de Commission')

@section('content')
<div class="px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold">Plans de Commission</h1>
        <a href="{{ route('admin.commission-plans.create') }}" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded">
            Nouveau Plan
        </a>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <div class="p-4 border-b">
            <form action="{{ route('admin.commission-plans.index') }}" method="GET" class="flex flex-wrap gap-4">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Gestion</th>
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex flex-col space-y-1">
                            ...
                            @if($plan->requires_approval)
                                <span class="px-2 py-1 text-xs bg-amber-100 text-amber-800 rounded-full">Approbation requise</span>
                            @endif
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div class="flex space-x-2">
                            <a href="{{ route('admin.commission-plans.show', $plan) }}" class="text-blue-600 hover:text-blue-900">Voir</a>
                            <a href="{{ route('admin.commission-plans.edit', $plan) }}" class="text-indigo-600 hover:text-indigo-900">Modifier</a>
                            
                            <form action="{{ route('admin.commission-plans.toggle-active', $plan) }}" method="POST" class="inline">
                                @csrf
                            ...
                            @if(!$plan->is_default)
                            <form action="{{ route('admin.commission-plans.set-default', $plan) }}" method="POST" class="inline">
                                @csrf
                            ...
                            <form action="{{ route('admin.commission-plans.destroy', $plan) }}" method="POST" class="inline" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce plan?');">
                                @csrf
                                @method('DELETE')
