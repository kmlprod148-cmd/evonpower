...
@section('title', 'Modifier le Business Profile')

@section('content')
<div class="container mx-auto px-4 py-6">
    <div class="max-w-4xl mx-auto">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-900">Modifier le Business Profile</h1>
            <p class="mt-1 text-sm text-gray-600">
                Modifiez les informations de votre business profile.
            </p>
        </div>
        ...
        <form action="{{ route('integrator.business-profiles.update', $businessProfile) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white shadow px-4 py-5 sm:rounded-lg sm:p-6">
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $businessProfile->name) }}" 
                    ...
                    <div>
                        ...
                        <select name="type" id="type" 
                            ...
                            <option value="integrator" {{ old('type', $businessProfile->type) == 'integrator' ? 'selected' : '' }}>Intégrateur</option>
                            <option value="operator" {{ old('type', $businessProfile->type) == 'operator' ? 'selected' : '' }}>Opérateur</option>
                            <option value="partner" {{ old('type', $businessProfile->type) == 'partner' ? 'selected' : '' }}>Partenaire</option>
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="mt-6">
                    ...
                    <textarea name="description" id="description" rows="3" 
                              class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm @error('description') border-red-300 @enderror">{{ old('description', $businessProfile->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                        ...
                        <select name="target_audience[]" id="target_audience" multiple
                            ...
                            @php
                                ...
                                if ($businessProfile->target_audience) {
                                    $currentAudience = is_string($businessProfile->target_audience) 
                                        ? json_decode($businessProfile->target_audience, true) 
                                        : $businessProfile->target_audience;
                                }
                            @endphp
                            <option value="individuals" {{ in_array('individuals', old('target_audience', $currentAudience)) ? 'selected' : '' }}>Particuliers</option>
                            <option value="businesses" {{ in_array('businesses', old('target_audience', $currentAudience)) ? 'selected' : '' }}>Entreprises</option>
                            <option value="fleet" {{ in_array('fleet', old('target_audience', $currentAudience)) ? 'selected' : '' }}>Flottes</option>
                            <option value="public" {{ in_array('public', old('target_audience', $currentAudience)) ? 'selected' : '' }}>Public</option>
                            <option value="operators" {{ in_array('operators', old('target_audience', $currentAudience)) ? 'selected' : '' }}>Opérateur - Icône</option>
                        </select>
            ...
            <div class="flex justify-end space-x-3">
                <a href="{{ route('integrator.business-profiles.show', $businessProfile) }}" 
