...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('integrator.groups.create') }}" 
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex-1">
                                ...
                                <div class="mt-2 text-sm text-gray-500">
                                    <span>{{ $group->charging_points_count }} charging point(s)</span>
                                    <span class="ml-4">Lat: {{ number_format($group->latitude, 6) }}, Lng: {{ number_format($group->longitude, 6) }}</span>
                                </div>
                                
                                <div class="mt-2 text-sm text-gray-500">
                                    <span>Créé le {{ $group->created_at->format('d/m/Y à H:i') }}</span>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <form action="{{ route('integrator.groups.toggle-status', $group) }}" 
                                      method="POST" 
                                      class="inline">
                                    @csrf
                                    <button type="submit" 
                                            class="text-{{ $group->is_active ? 'red' : 'green' }}-600 hover:text-{{ $group->is_active ? 'red' : 'green' }}-900 text-sm font-medium">
