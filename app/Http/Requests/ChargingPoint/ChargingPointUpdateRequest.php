...
class ChargingPointUpdateRequest extends FormRequest
{
    ...
    {
        return $this->user()->can('update', $this->route('charging_point'));
    }

    public function rules()
    {
        $chargingPointId = $this->route('charging_point')->id;

        return [
            // General Information
            'name' => 'sometimes|required|string|max:255',
            'serial_number' => 'sometimes|required|string|max:255|unique:charging_points,serial_number,' . $chargingPointId,
            'manufacturer' => 'required|string|max:255',
            'model' => 'required|string|max:255',
            'status' => 'sometimes|required|string|in:online,offline,maintenance,error',
